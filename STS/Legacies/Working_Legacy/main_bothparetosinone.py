...
def extract_raw_pareto_front(df_cp, df_trf):
    ...
    return merged[is_pareto].sort_values(by='cp', ascending=False).reset_index(drop=True)

def generate_pareto_figure(df_pareto, df_raw_pareto=None):
    """
    Stage 14: Plots Pareto fronts (Pre-NSGA-II Raw CFD vs Post-NSGA-II Optimized).
    """
    print(" -> Generating Dual Pareto Front Comparison Plot...")
    ...
    # Highlight the absolute best designs
    ...
    plt.title("Risk-Aware NSGA-II vs Pre-Optimization Pareto Front", fontsize=15, fontweight='bold')
    ...
    # Set y-axis upper bound to 0.45
    plt.ylim(top=0.45)
    
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend(loc='lower right')
    
    filepath = os.path.join("figures", "Pareto_Front_Analysis.png")
    plt.tight_layout()
    plt.savefig(filepath, dpi=300)
    plt.close()
    print(f"    Saved: {filepath}")
...
def main():
    ...
    # 5. Export and Plot Pareto Front
    if df_pareto is not None:
        ...
        df_raw_pareto = extract_raw_pareto_front(df_cp, df_trf)
        generate_pareto_figure(df_pareto, df_raw_pareto)
