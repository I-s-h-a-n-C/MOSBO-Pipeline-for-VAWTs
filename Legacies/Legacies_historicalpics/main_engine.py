...
def generate_historical_pareto_figure(current_df, history_dir):
    ...
    # 1. Load, sort, and plot all historical runs with fading transparency
    ...
    for i, file in enumerate(history_files):
        try:
            ...
            if 'Pred_TRF' in hist_df.columns and 'Pred_Cp' in hist_df.columns:
                ...
                # Dynamic Alpha: Oldest is 0.2 (high transparency), newest is 0.7 (low transparency)
                if n_files > 1:
                    alpha_val = 0.20 + (0.50 * (i / (n_files - 1)))
                else:
                    alpha_val = 0.40
                
                # Plot historical runs as scatter points colored by uncertainty, similar to optimized plot
                if 'Sigma_Cp' in hist_df.columns:
                    plt.scatter(hist_df['Pred_TRF'], hist_df['Pred_Cp'], 
                                c=hist_df['Sigma_Cp'], cmap='viridis', 
                                s=40, alpha=alpha_val, edgecolor='none')
                else:
                    # Fallback if historical run doesn't have Sigma_Cp
                    plt.scatter(hist_df['Pred_TRF'], hist_df['Pred_Cp'], 
                                color='gray', s=40, alpha=alpha_val, edgecolor='none')
                    
        except Exception:
            pass # Ignore corrupted or unrelated CSVs
            
    # 2. Plot the current latest run prominently on top, matching the optimized plot style
    scatter = plt.scatter(current_df['Pred_TRF'], current_df['Pred_Cp'], 
                          c=current_df['Sigma_Cp'], cmap='viridis', 
                          s=80, alpha=0.9, edgecolor='k', label='Latest Optimization Run')
    ...
    # Set y-axis upper bound to match other plots
    plt.ylim(top=0.4)
...
def main():
    ...
    if df_pareto is not None:
        ...
        # --- NEW: Historical Archiving Logic ---
