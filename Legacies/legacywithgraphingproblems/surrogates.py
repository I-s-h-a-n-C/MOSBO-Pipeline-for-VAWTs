...
class SurrogateBuilder:
    ...
    - Stage 5: Model Training (Automated GP Kernel Search or Random Forest)
    ...
    def train_random_forest(self, X_train_scaled, y_train_scaled):
        """Trains a Random Forest Regressor using all available data."""
        start_time = time.time()
        print(f"[{self.target_name}] Training Random Forest Regressor on {len(X_train_scaled)} points...")
        
        # 100 trees is generally a great balance of accuracy and speed
...
def build_surrogates(df_cp, df_trf):
    ...
    # -------------------------------------------------------------------------
    # 1. Aerodynamic Power Coefficient (Cp) Surrogate - RANDOM FOREST
    # -------------------------------------------------------------------------
    print("\n[STEP 1/2] Building Aerodynamic Surrogate (Cp)...")
    cp_builder = SurrogateBuilder('Cp', ['thickness', 'twist', 'solidity', 'tsr'], model_type='rf')
    X_train_c, X_test_c, y_train_c, y_test_c, X_full_c, y_full_c = cp_builder.prepare_data(df_cp, 'cp')
    
    cp_builder.train_random_forest(X_train_c, y_train_c)
