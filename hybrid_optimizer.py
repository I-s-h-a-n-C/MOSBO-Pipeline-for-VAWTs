...
class GenerationProgressBar(Callback):
    """Custom callback to render a tqdm progress bar during NSGA-II evolution."""
...
def run_optimization(cp_surrogate, trf_surrogate, df_cp):
    ...
    # Enable save_history=True to record population state across all generations
    res = pymoo_minimize(
        problem,
        algorithm,
        termination,
    ...
    df_pareto = df_pareto[(df_pareto['Pred_TRF'] <= 1.5) & (df_pareto['Pred_Cp'] >= 0.20)]
